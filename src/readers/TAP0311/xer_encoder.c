...
const char* global_buf = 0;
size_t size_buf = 0;

void print_part(char *str, int size);
/*
 * The XER encoder of any type. May be invoked by the application.
 */
asn_enc_rval_t
xer_encode(const asn_TYPE_descriptor_t *td, 
           const void *sptr,
...
/*
 * This is a helper function for xer_assign, which directs all incoming data
 * into a buffer
 */
static int
...
xer_assign(const char * const buffer, const asn_TYPE_descriptor_t *td, const void *sptr){
	...
	er = xer_encode(td, sptr, XER_F_BASIC, xer__assign2str, buffer);
	if(er.encoded == -1)
		return -1;
